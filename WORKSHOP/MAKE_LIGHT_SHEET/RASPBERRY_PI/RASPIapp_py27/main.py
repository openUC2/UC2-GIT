...
class Fluidiscope(BoxLayout):
    init_var = ListProperty([fg.config['light']['intensity_expt'],str(fg.config['motor']['stepsize_z']),fg.config['light_patterns']['active_p1'],fg.config['light_patterns']['active_p2'],fg.config['light_patterns']['active_p3'],fg.config['light_patterns']['active_p4']])
    ...
    def slider_setNA(self, instance):
        if not fg.my_dev_flag:
            ...
            fluidiscopeIO.update_matrix(self, instance, ignore_NA=True, sync_only=False)
