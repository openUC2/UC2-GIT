...
void callback(char *topic, byte *message, unsigned int length)
{
  ...
  if (std::string(topic) == stopicREC)
  {
    ...
    {
      drive_x((int)INSTS[0] * 10, myspeed, 1);
    }
    else if (strcmp(CMD, COMMANDSET[1]) == 0)
    {
      drive_y((int)INSTS[0] * 10, myspeed, 1);
    }
    else if (strcmp(CMD, COMMANDSET[2]) == 0)
    {
      drive_z((int)INSTS[0] * 10, myspeed, 1);
    }
    else
    {
...
void drive_x(int nsteps, unsigned int motorSpeed, int mydirection) {
...
void drive_y(int nsteps, unsigned int motorSpeed, int mydirection) {
  int i_iter = nsteps / n_substeps;
  if (mydirection > 0) {
    for (int i = 0; i < i_iter; i++) {
      drive_left(motorSpeed, motorPin_X, n_substeps);
...
void drive_z(int nsteps, unsigned int motorSpeed, int mydirection) {
  ...
  if (mydirection > 0) {
      ...
      drive_right(motorSpeed, motorPin_Z, nsteps);
  }
  else {
      drive_left(motorSpeed, motorPin_X, nsteps);
      drive_left(motorSpeed, motorPin_Y, nsteps);
      drive_left(motorSpeed, motorPin_Z, nsteps);
  }
